...
# shellcheck disable=SC2016,SC2046,SC3043
...
add_stream_block() {
    local conffile="/etc/angie/angie.conf"

    if grep -q -E "\s*stream\s*\{" "$conffile"; then
        entrypoint_log "$SCRIPT_NAME: $conffile contains a stream block; include $stream_output_dir/*.conf to enable stream templates"
    else
        # check if the file can be modified, e.g. not on a r/o filesystem
        touch "$conffile" 2> /dev/null || {
            entrypoint_log "$SCRIPT_NAME: info: can not modify $conffile (read-only file system?)"
            exit 0
        }
        entrypoint_log "$SCRIPT_NAME: Appending stream block to $conffile to include $stream_output_dir/*.conf"
        cat << END >> "$conffile"
...
auto_envsubst() {
    ...
    if [ ! -w "$output_dir" ]; then
        entrypoint_log "$SCRIPT_NAME: ERROR: $template_dir exists, but $output_dir is not writable"
        return 0
    fi
    find "$template_dir" -follow -type f -name "*$suffix" -print | while read -r template; do
        ...
        entrypoint_log "$SCRIPT_NAME: Running envsubst on $template to $output_path"
    ...
    if test -n "$(find "$template_dir" -name "*$stream_suffix" -print -quit)"; then
        ...
        if [ ! -w "$stream_output_dir" ]; then
            entrypoint_log "$SCRIPT_NAME: ERROR: $template_dir exists, but $stream_output_dir is not writable"
            return 0
        fi
        add_stream_block
